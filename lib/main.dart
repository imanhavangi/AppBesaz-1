import 'dart:convert';
import 'package:appbesaz/goftino.dart';
...
import 'package:appbesaz/modules/EmailModule/emailModule.dart';
import 'package:appbesaz/modules/ExitModule/exitModule.dart';
import 'package:appbesaz/modules/OnlineGalleryModule/onlineGalleryModule.dart';
import 'package:appbesaz/modules/OfflineGalleryModule/offlineGalleryModule.dart';
...
Future<AparatProfile> fetchAparatProfile(String profileCode) async {
  final response = await http.get(Uri.parse(
      'https://www.aparat.com/etc/api/profile/username/$profileCode'));

  if (response.statusCode == 200) {
    return AparatProfile.fromJson(jsonDecode(response.body));
  } else {
...
class AparatProfile {
  ...
  factory AparatProfile.fromJson(Map<String, dynamic> json) {
    return AparatProfile(
      data: json['profile'],
      // link: json['link']
    );
  }
...
late Future<AparatProfile> futureAparatProfile;

Future<AparatVideo> fetchAparatVideo(String videoCode) async {
  final response = await http.get(
      Uri.parse('https://www.aparat.com/etc/api/video/videohash/$videoCode'));

  if (response.statusCode == 200) {
    return AparatVideo.fromJson(jsonDecode(response.body));
  } else {
    throw Exception('Failed to load album');
  }
}
...
class AparatVideo {
  ...
  factory AparatVideo.fromJson(Map<String, dynamic> json) {
    return AparatVideo(
      data: json['video'],
      // link: json['link']
    );
...
late Future<AparatVideo> futureAparatVideo;

Future<AparatChannel> fetchAparatChannel(String channel) async {
  final response = await http.get(Uri.parse(
      'https://www.aparat.com/etc/api/videoByUser/username/$channel/perpage/50'));

  if (response.statusCode == 200) {
    return AparatChannel.fromJson(jsonDecode(response.body));
  } else {
    throw Exception('Failed to load album');
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  SiteModule si = new SiteModule(
      id: 0,
      index: 0,
      title: 'افزونه سایت',
      siteAddress: 'https://www.google.com');
  CallModule c =
      new CallModule(id: 1, index: 1, title: 'افزونه تماس', phoneNumber: '112');
  AparatModule aparatModule = new AparatModule(
      ...
      aparatType: 2,
      data: "5zOim,qBXf5");
  AparatModule a1 = new AparatModule(
      id: moduleList.length,
      index: moduleList.length,
      title: 'افزونه آپارات با صدای سگ',
      uiList: 1,
      uiOne: 1,
      aparatType: 3,
      data: "Amirnasiri1998");
  AboutusModule au1 = new AboutusModule(
    ...
    title: 'اپ بساز',
    text:
        'ما استارتاپ اپ بساز می باشیم و قرار است تمامی رقبا را پشت سر گذاشته و از آن ها جلو بزنیم، ما میتوانیم با ایجاد شعبه های بین المللی بینی اپتو را به خاک بمالیم (اپتو می گوید: بچه بیا پایین سرمون درد گرفت)',
    cardWidgetList: [
      CardWidget(
        title: 'ایمان هاونگی',
        text:
            'تیم اپ بساز متشکل از 6 نفر می باشد. بچه های تیم اپ بساز در اکثر اوقات روز صدای سگ میدهند. هر شب ساعت 10 می توانید زوزه های ایمان را در دیسکورد بشنوید.',
        // text: 'salam',
        image: 'assets/iman.jpg',
      ),
      CardWidget(
        title: 'سجاد ضیائی',
        text: 'hello',
  ...
  OnlineGalleryModule ogm1 = OnlineGalleryModule(
      id: moduleList.length,
      index: moduleList.length,
      title: 'گالری آنلاین',
      imageList: [
        ImageAB(
            title: 'ایمان', link: 'https://namnamak.com/images/up/129/971.jpg'),
        ImageAB(
            title: 'قاسم',
            link:
                'https://static2.jamaran.news/servev2/9yALkx17U3no/lLl5vbV5w7c,/photo_2018-05-23_11-54-02.jpg'),
        ImageAB(
            title: 'title',
            link:
                'https://images-na.ssl-images-amazon.com/images/I/51rwBMmnPYL._SX384_BO1,204,203,200_.jpg'),
        ImageAB(
            title: 'title',
            link: 'https://m.media-amazon.com/images/I/51pYggo33jL.jpg'),
        ImageAB(
            title: 'title',
            link: 'https://wallpaperaccess.com/full/3746815.jpg'),
        ImageAB(
            title: 'title',
            link:
                'https://howtodrawa.org/wp-content/uploads/2020/12/anime-girl3-898x1024.png'),
        ImageAB(
            title: 'title',
            link:
        ...
        ImageAB(
            title: 'title',
            link:
                'https://b2cproductionsite-be80wkthrgjjkmzuip0.netdna-ssl.com/sites/b2c/files/styles/orange_blog_in_picture/public/generated/blog/memes.png'),
        ImageAB(
            title: 'title',
            link:
                'https://ichef.bbci.co.uk/news/1024/branded_news/7CC2/production/_118283913_29xp-meme-mobilemasterat3x-v3.jpg')
      ]);
  OfflineGalleryModule ofgm1 = OfflineGalleryModule(
      id: moduleList.length,
      index: moduleList.length,
      title: 'گالری آفلاین',
      imageList: [
        ImageOffline(title: 'title', name: 'assets/iman.jpg'),
        ImageOffline(title: 'title', name: 'assets/back.jpg'),
        ImageOffline(title: 'title', name: 'assets/back1.jpg'),
        ImageOffline(title: 'title', name: 'assets/back2.jpg'),
        ImageOffline(title: 'title', name: 'assets/facebook.jpg')
      ]);
  EmailModule em1 = EmailModule(
    id: moduleList.length,
    index: moduleList.length,
    title: 'ایمیل مستقیم',
  ...
  ExitModule exm1 = ExitModule(
      id: moduleList.length, index: moduleList.length, title: 'خروج');
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          // Navigator.push(
          //     context, MaterialPageRoute(builder: (context) => Goftino()));
          // setState(() {
          //   cmm.addModule(c);
